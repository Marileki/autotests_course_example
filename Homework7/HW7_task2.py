...
class PersonInfo:
    ...
    def __init__(self, fio, age, *podr):
        self.fio = fio
        self.age = age
        self.podr = podr

    def short_name(self):
        """Возвращает строку с именем в формате Фамилия И."""
        b = self.fio.find(" ")
        str1 = self.fio[b + 1:] + ' ' + self.fio[0] + '.'
        return str1

    def path_deps(self):
        """Возвращает строку, содержащую путь от Головного до конечного подразделения"""
        str2 = ''
        str2 += " --> ".join(self.podr)
        return str2

    def new_salary(self):
        """Возвращает рассчитанную зарплату"""
        import collections
        s = "".join(self.podr)
        simv3 = collections.Counter(s).most_common(3)
        m = 0
        for i in simv3:
            m += i[1]
        salary = 1337 * m * self.age
        return salary
