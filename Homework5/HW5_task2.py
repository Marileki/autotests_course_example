...
def repeats(our_str):
    """
    Формирует новую строку с количеством символов из полученной строки
    :param our_str: Изначальная строка
    :return: Новая строка, показывает сколько раз встречался каждый символ из неё
    """
    new_str = ""
    b = dict.fromkeys(our_str, 0)
    for i in our_str:
        b[i] += 1
        new_str = new_str + i + '_' + str(b[i])
    return new_str
