...
def everything_for_your_cat(cats_data):
    """
    Формирует словарь, где Ключ - Имя Фамилия покупателя, Значения - его питомцы, и возвращает список в формате строки
    :param cats_data: Кортеж покупателей и питомцев
    :return: Данные о покупателях и питомцах построчно
    """
    b = {}
    our_str = ''
    for k, v, f, i in cats_data:
        b.setdefault((f + " " + i), []).append((k, v))

    for p, l in b.items():
        our_str = our_str + p + ": "
        for h in l:
            our_str = our_str + h[0] + ', ' + str(h[1]) + '; '
        our_str = our_str.rstrip("; ") + '\n'
    return our_str
