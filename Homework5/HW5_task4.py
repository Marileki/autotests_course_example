...
def scrabble(word):
    """
    Считает очки в игре "Эрудит", по заданному словарю, где Ключ - буква, Значение - очки за букву
    :param word: Полученное слово
    :return: Очки
    """
    alfavit = {'а': 1, 'б': 3, 'в': 1, 'г': 3, 'д': 2, 'е': 1, 'ё': 1, 'ж': 5, 'з': 5, 'и': 1, 'й': 4, 'к': 2, 'л': 2,
               'м': 2, 'н': 1, 'о': 1, 'п': 2, 'р': 1, 'с': 1, 'т': 1, 'у': 2, 'ф': 8, 'х': 5, 'ц': 5, 'ч': 5, 'ш': 8,
               'щ': 10, 'ъ': 15, 'ы': 4, 'ь': 3, 'э': 8, 'ю': 8, 'я': 3}
    points = 0
    for i in word:
        i = i.lower()
        points = points + alfavit.get(i)
    return points
